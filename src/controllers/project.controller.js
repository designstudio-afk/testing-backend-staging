...
export const createProject = async (req, res, next) => {
  try {
    ...
    // Add sub-categories to project
    for (const subCategoryId of sub_category_ids) {
      await pool.query(
        "INSERT INTO project_sub_categories (project_id, sub_category_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
        [projectId, subCategoryId],
      )
    }

    // Get the created project with its sub-categories
...
export const addSubCategoriesToProject = async (req, res, next) => {
  try {
    const { projectId } = req.params
    const { sub_category_ids } = req.body

    if (!Array.isArray(sub_category_ids) || sub_category_ids.length === 0) {
      return res.status(400).json({ error: "sub_category_ids array is required" })
    }
    ...
    // Insert sub-categories (skip duplicates)
    for (const subCategoryId of sub_category_ids) {
      await pool.query(
        "INSERT INTO project_sub_categories (project_id, sub_category_id) VALUES ($1, $2) ON CONFLICT DO NOTHING",
        [projectId, subCategoryId]
    ...
    // Get all sub-categories for this project
    const result = await pool.query(
      `SELECT sc.* FROM sub_category sc 
       INNER JOIN project_sub_categories psc ON sc.id = psc.sub_category_id 
       WHERE psc.project_id = $1 ORDER BY sc.sub_category_name ASC`,
      [projectId]
    )
...
export const getProjectSubCategories = async (req, res, next) => {
  try {
    const { projectId } = req.params
...
export const removeSubCategoryFromProject = async (req, res, next) => {
  try {
    const { projectId, subCategoryId } = req.params

    const result = await pool.query(
      "DELETE FROM project_sub_categories WHERE project_id = $1 AND sub_category_id = $2 RETURNING *",
      [projectId, subCategoryId]
    )

    if (result.rows.length === 0) {
      return res.status(404).json({ error: "Sub-category not found for this project" })
    }
...
export const getProjectsByFilter = async (req, res, next) => {
  try {
    ...
    if (subCategoryId) {
      query += `
        INNER JOIN project_sub_categories psc ON p.id = psc.project_id
        INNER JOIN sub_category sc ON psc.sub_category_id = sc.id
      `
      query += " WHERE sc.id = $1"
